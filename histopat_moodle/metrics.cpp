...
fsiv_chisquared_dist(const cv::Mat & h1, const cv::Mat & h2)
{
    ...
    //Dont use compareHist XDDDDD not cool
...
compute_mean_recognition_rate(const cv::Mat& cmat, std::vector<int> const& categories)
{
    ...
    MMR = MMR / static_cast<float>(cmat.rows); //TODO: Check if its needed to convert to float
    ...
    MMR = MRR / static_cast<float>(categories.size());
...
compute_accuracy(const cv::Mat& cmat, std::vector<int> const& categories)
{
    ...
    //TODO: Check if correct
    cv::Mat row_sum;
    cv::reduce(cmat, row_sum, 1, CV_REDUCE_SUM, CV_32F);
    if (cv::sum(row_sum)[0] != 0) // Not dividing by zero
    {
        float aux = 0.0;
        for(int i=0; i<categories.size(); ++i)
        {
    ...
    for(int i=0; i<categories.size(); ++i)
    {
        ...
        false += cv::sum(cmat.row(categories[i]))[0] - cmat.at<float>(categories[i], categories[i]);
    }

    if (correct + false != 0) // Not dividing by zero
    {
